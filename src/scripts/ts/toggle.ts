 ...
 */
function initToggleSectionByQuery(
    titleQuery  : string,
    targetQuery : string,
    initHide    : boolean = true
){
    ...
    if (titleElement && targetElement && titleElement.length==targetElement.length){
        for(let i = 0; i < titleElement.length; i++)
            initToggleSection(titleElement[i] as HTMLElement, targetElement[i] as HTMLElement, initHide);
    }
}
 ...
 */
function initToggleSectionsAll(
    initHide: boolean = true
){
    const sectionElements = document.querySelectorAll("main > div");
    sectionElements.forEach(section => {
        const titleElement = section.querySelector("h2") as HTMLElement;
        const targetElement = titleElement.nextElementSibling as HTMLElement;
 ...
 * @returns {void}
 */
function initToggleSectionsByClass(
    titleClass  : string  = "toggle-title",
    targetClass : string  = "toggle-content",
    initHide    : boolean = true
){
    const titleElements = document.querySelectorAll(`.${titleClass}`);
    titleElements.forEach(titleElement => {
        let nextElement = titleElement.nextElementSibling as HTMLElement;
        while (nextElement && !nextElement.classList.contains(targetClass)) {
            nextElement = nextElement.nextElementSibling as HTMLElement;
        }
        if (nextElement) {
            initToggleSection(titleElement as HTMLElement, nextElement, initHide);
        } else {
